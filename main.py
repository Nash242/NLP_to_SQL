...
port=1433
connection_string = f'mssql+pymssql://{username}:{encoded_password}@{server_name}:{port}'
...
def getquery():
    ...
    try:
        if request.method=='POST':
            ...
            print(query)
            conn=master_session[session['user']]['conn']
            html_table,graph_html,sqlquery= main(query,conn)
...
def conectdb():
    ...
    try:
        if (request.method == 'POST'):
            ...
            conn, connection_string, engine=connectmysqldb(db_user,db_password,db_host,db_port,db_name)
            print("12345",session)
    ...
    try:
        if (request.method == 'GET'):
            ...
            if value is not None:
                schema=alldata(master_session[session['user']]['metadata']['db_name'])
...
"""
    ...
    g1 = graph
    print(g1)
    # graph_html = pio.to_html(g1, full_html=False,include_plotlyjs=False,config={'responsive': True},default_height="338px",default_width="100%",div_id="mygraph")
    print(g1)
    conn.close()
    newdf= df.to_html()
